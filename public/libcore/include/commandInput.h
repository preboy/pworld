...
{
    PARAM_CNT = 10,         // 参数最大数量
    NAME_LEN = 16,          // 命令字符串的长度
    HELP_LEN = 64,          // 帮助信息长度
    DESC_LEN = 64,          // 描述信息的长度   
};

class CCommandInput;
using CMD_FUNC = int (CCommandInput::*)(int argc, char argv[PARAM_CNT][NAME_LEN]);
...
private:
    ...
    {
        uint32      argc = 0;
        char        argv[PARAM_CNT][NAME_LEN] = { { 0 } };
...
private:
    ...
    int OnQuit(int argc, char argv[PARAM_CNT][NAME_LEN]);
    int OnHelp(int argc, char argv[PARAM_CNT][NAME_LEN]);
    int OnList(int argc, char argv[PARAM_CNT][NAME_LEN]);
